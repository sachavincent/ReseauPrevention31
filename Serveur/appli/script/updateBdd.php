...
function creerCSV($cle, $table, $bdd, $idFilAnnonce, $device_id){
    ...
    switch($table){
        ...
        break;
        case 'Conseil':
            $requestSQL = $bdd->query('SELECT * FROM Conseil WHERE corbeille=0');
...
function besoinMaj($cle, $table, $derniereMAJ, $bdd, $idFilAnnonce){
    ...
    switch($table){
        ...
        break;
        case 'Conseil':
            $requestSQL = $bdd->prepare('SELECT * FROM Conseil WHERE updated_at > ? AND corbeille=0');
            $requestSQL->execute(array($derniereMAJ));
            $resultatRequete = $requestSQL->fetchAll();
        break;
        case 'FilDeDiscussion':
        ...
        break;
        case 'Annonce':
            $requestSQL = $bdd->prepare('SELECT * FROM Annonce WHERE created_at > ? AND idAnnonce = ? AND corbeille = 0');
            foreach($idFilAnnonce['annonce'] as $idAnnonce){
...
if (empty($info['derniere_mise_a_jour']) OR empty($info['device_id'])){//Si date de derniere mise a jour non renseignée
    ...
    if (empty($resultatRequete['cle'])){
        $retour['error'] = 101;
            ...
            $tables = array('CodeActivite', 'Commune', 'Conseil', 'FilDeDiscussion', 'MessagePrive', 'Annonce');
